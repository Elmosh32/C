#ifndef __GEN_HASH_MAP_H__
#define __GEN_HASH_MAP_H__
...
typedef enum Map_Result
{
	...
	MAP_EMPTY_ERROR			 /**< Empty hashMap error 		*/
} Map_Result;

/**
 * @brief Function that generate index for given key.
 * @param[in] _key - pointer to key.
 * @return size_t number: index
 */
typedef size_t (*HashFunction)(const void *_key);

/**
 * @brief Function that used to compare elements.
 * @param[in] _firstKey - pointer to first element.
 * @param[in] _secondKey - pointer to second element.
 * @return int number: 1 if _firstKey == _secondKey , otherwise zero.
 */
typedef int (*EqualityFunction)(void *_firstKey, void *_secondKey);

/**
 * @brief Function that used to compare elements.
 * @param[in] _key - pointer to the key to serve as index.
 * @param[in] _value - pointer to value to associate with the key.
 * @param[in] _context -
 * @return int number: 1 or 0 => 1 to continue forech iteration 0 to stop.
 */
 ...
 * @retval  MAP_ALLOCATION_ERROR on failed to allocate key-value pair
